...
const esquema = z.strictObject({
  ...
  dados: z.strictObject({ experienciaId: z.string().uuid(), limite: z.number().int().positive().optional() }),
})

export class GerarListaEspera {
  constructor(private readonly d: any) {}
  async executar(e: unknown) {
    const { contexto, dados } = validarEntrada(esquema, e)
    return this.d.repositorio.listarEspera({ experienciaId: dados.experienciaId, negocioId: contexto.negocioId, limite: dados.limite })
  }
}